package dk.trustworks.aggregater.repository;

import dk.trustworks.aggregater.model.Work;
...
public class WorkItemsRepository {
    ...
    public Single<List<Work>> getAllWork() {
        String sql = "select w.id, w.registered, w.workduration, w.useruuid, w.workas, cc.rate, t.uuid, p.uuid, c.uuid from " +
                "            work as w " +
        ...
        return connect().flatMap((SQLConnection connection) -> connection.rxQuery(sql)
                .map(rs -> rs.getRows().stream().map(Work::new).collect(Collectors.toList()))
